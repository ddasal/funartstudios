...
class Typical(models.Model): 
    ...
    notes = models.TextField(null=True, blank=True, default='I prefer to work up to X events per week.  I may be able to cover some last minute private parties or emergency coverages. Just ask.')
...
class RequestStatus(models.TextChoices):
    PENDING = 'p', 'Pending Request'
    COMPLETE = 'c', 'Approved/Complete'
    ABORTED = 'a', 'Aborted Request'
    DENIED = 'd', 'Request Denied'

### mostly a duplicate of Typical for Change Requests

class ScheduleChange(models.Model): 
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    approved_by = models.ForeignKey(User, on_delete=CASCADE, null=True, blank=True, related_name='approved_by')
    start_date = models.DateField(null=False, blank=False, default=timezone.now, auto_now=False, auto_now_add=False)
    ...
    sunday = models.IntegerField(default=0, null=False, blank=False)
    timestamp = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(User, on_delete=CASCADE, null=True, blank=True, related_name='schedule_edit_created_by')
    updated_by = models.ForeignKey(User, on_delete=CASCADE, null=True, blank=True, related_name='schedule_edit_updated_by')
    updated = models.DateTimeField(auto_now=True)
    active = models.BooleanField(default=True)
    notes = models.TextField(null=True, blank=True, default='I prefer to work up to X events per week.  I may be able to cover some last minute private parties or emergency coverages. Just ask.')
